...
The dataset that we will use throughout this analysis comes from the [UCI Machine Learning Repository](https://archive.ics.uci.edu/ml/datasets/Beijing+PM2.5+Data) gathered by Liang, X. et al (2015). The dataset contains hourly meteorological data from 2010 Jan 1st to 2014 Dec 31st of US Embassy in Beijing, which includes information such as the PM2.5 concentration, dew point, temperature, wind speed, etc. Among those features, listed below are our interested variables and their corresponding units: 
  ...
  1. PM2.5 concentration ($\mu$g/m$^3$);
  2. Hour of the day;
  3. Dew point ($^\circ$C);
...
```{r}
data <- read.csv("../data/beijing-pm25.csv")
```
...
data %>%
  ...
  knitr::kable(caption = "PM2.5 concentration by part of the day") %>%
...
data %>%
  ...
  knitr::kable(caption = "PM2.5 concentration by temperature category") %>%
  kable_styling(latex_options = "HOLD_position")
```
...
data %>%
  ...
  knitr::kable(caption = "PM2.5 concentration by dew point category") %>%
  kable_styling(latex_options = "HOLD_position")
```
...
data %>%
  ...
  knitr::kable(caption = "PM2.5 concentration by wind speed category") %>%
  kable_styling(latex_options = "HOLD_position")
```
...
The coeffiecients of wind speed, dew point, and temperature perfectly aligns with our previous results:
  ...
  1. for every 1 m/s increase in wind speed, we expect a 0.26 $\mu$g/m$^3$ decrease in PM2.5 concentration;
  2. for every 1 $^\circ$C increase in dew point (more moisture in the air), we expect a 5.47 $mu$g/m$^3$ increase in PM2.5 concentration;
  3. for every 1 $^\circ$C increase in temperature, we expect a 6.60 $\mu$g/m$^3$ decrease in PM2.5 concentration.
...
Recall that earlier in the *Data cleaning and wrangling* section, we have created categorical varibles for PM2.5 concentrations, i.e., "good", "moderate", "unhealthy", "very unhealthy", and "hazardous". Now we will build decision trees that utilizes meteological factors to predict this variable. The reason that we choose this method is that decision trees are generally easier to interpret and can be used as a good travel guideline for everyone. 
...
```{r}
data_size <- nrow(data)
train_idx <- sample(1:data_size, size = 0.7 * data_size)
...
```{r fig.cap="PM2.5 concentration level decision tree"}
# Build a simple decision tree
treefit <- rpart(
  as.factor(air_quality) ~ dew_pt + temp + wind_spd + day_part, 
  method = "class",
  data = data_train
)
rpart.plot(treefit)
...
From the decision tree plot above, we can see that the decision boundaries aligns perfectly with our previous results from the regression model. To be specific, the decision boundaries show that lower dew pooint and temperature indeed leads to lower PM2.5 concentrations.

```{r results=FALSE}
# Make predictions on the test dataset, compute accuracy
pred_tree <- predict(treefit, data_test, type = "class")
mean(pred_tree == data_test$air_quality)
```
...
mean(pred_rf == data_test$air_quality)
```
